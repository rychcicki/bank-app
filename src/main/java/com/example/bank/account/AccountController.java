...
class AccountController {
    ...
    AccountDTO createMyBankAccount(@PathVariable @Min(1) Long clientId) {
        return accountService.createMyBankAccount(clientId);
    }

    @PostMapping("/polish/{clientId}")
    AccountDTO createPolishAccounts(@PathVariable @Min(1) Long clientId) {
        return accountService.createPolishAccount(clientId);
    }

    @PostMapping("/foreign/{clientId}")
    AccountDTO createForeignAccount(@PathVariable @Min(1) Long clientId) {
